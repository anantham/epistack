...
test("research route keeps discovery separate from evidence promotion", async () => {
  ...
  assert.match(dashboard, /The agent proposes\. The evidence graph does not update silently/);
  assert.match(dashboard, /Inspect atomic results/);
  assert.match(dashboard, /Extract abstract results/);
  ...
  assert.match(promoteApi, /humanChecked !== true/);

...
export default function HostedDecomposition() {
  ...
  function download() {
    const blob = new Blob([JSON.stringify({ question: progress?.question, provider: 'Lyra public Responses', status: 'proposal-awaiting-human-review', decomposition: progress?.artifact, stages: progress?.results, contextualizationConducted: false, researchStarted: false }, null, 2)], { type: 'application/json' });
  ...
  return <main className="hosted-decomposition" style={{ maxWidth: 980, margin: 'auto', padding: '32px 24px', fontSize: 16, lineHeight: 1.65 }}>
    ...
    {!job && <><label htmlFor="question">Your question</label><textarea id="question" value={question} onChange={e => setQuestion(e.target.value)} rows={5} maxLength={5000} style={{ width: '100%', padding: 16, font: 'inherit', border: '1px solid #8291a5', borderRadius: 8 }} /><p>This question is processed through the owner’s Lyra service and ChatGPT. This preview allows ten investigations per day across the site.</p><button onClick={start} disabled={starting || question.trim().length < 12}>{starting ? 'Starting…' : 'Decompose question'}</button></>}

...
type Progress = { status: string; stage: number; question?: string; error?: string; artifact?: DecompositionArtifact };
...
export default function HostedDecomposition() {
  ...
  async function drive(value: string, refresh = false) {
    ...
    try {
      ...
      setProgress({ status: 'completed', stage: 3, question: response.prompt, artifact: response.decomposition });
  ...
  function download() {
    const blob = new Blob([JSON.stringify({ question: progress?.question, provider: 'Astra public Responses', status: 'proposal-awaiting-human-review', decomposition: progress?.artifact, contextualizationConducted: false, researchStarted: false }, null, 2)], { type: 'application/json' });

...
- [ ] **Ready for review (Claude, branch `feat/investigate-budget-settings` at `283d9be`, based on `7d39588`): research budget and model settings.** Compact backend status; per-role OpenRouter model pickers (search, paper reader, reviewer) with a server-proxied model list; Quick/Standard/Thorough effort with Standard as default; a $5 cap per run for everyone; capped web search (`max_uses`, `max_results`, `max_total_results`); per-call cost and duration recording with an empirical ETA. Verified locally, not deployed: typecheck and build passed; 129/129 tests after the final test update; one live OpenRouter recall run through the worktree dev server; cap refusal sends no request; 390px and 1440px layouts. Claim-card steering on Investigate is next; please keep avoiding overlapping edits to `app/api/recall`, `app/api/investigate`, `research-dashboard.tsx`, and `backend-settings.tsx`.

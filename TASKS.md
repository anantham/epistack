...
- [x] **Back up and integrate the current source work.** Canonical `main` is pushed to GitHub at `0f29744`, including the explicit `/decompose` route, research budget/model settings, Contextualize impact ledger, claim steering, human review overturns, and artifact JSON export. Sites source was reconciled with a non-force-pushed fast-forward commit `7ec6f00`; published version 93 is live.
...
- [ ] Close the hosted evidence gate gap. Re-run the live full-text review after the boundary-ellipsis fix and determine whether the remaining results pass exact passage, scope, and model-independence checks; do not weaken the evidence gate to manufacture an accepted record.
...
- [ ] Persist deterministic full-text rejection classes and bounded excerpt diagnostics so production runs distinguish malformed excerpts, missing passages, scope mismatches, hash mismatches, and incomplete reviewer coverage.
...
The current 150-test gate is strong for pure contracts and build safety, but it does not yet provide complete product or production confidence.
...
- [x] Add a unit regression test for model-added boundary ellipses; the literal passage gate remains strict after canonicalization.
